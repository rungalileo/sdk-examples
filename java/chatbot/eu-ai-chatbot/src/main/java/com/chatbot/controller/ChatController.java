...
public class ChatController {
    ...
    public ResponseEntity<ChatResponse> chat(@RequestBody ChatRequest request) {
        ...
        try {
            ...
            // Get the full messages array that was sent to the LLM (includes system message with RAG context!)
            List<Map<String, String>> fullMessages = (completionResult != null) ? completionResult.getFullMessages() : null;
            
            // Invoke Protect on output after getting response
            ...
            if (galileoService.isProtectEnabled() && response != null) {
                ...
                // Pass the full messages array to Protect (same as what LLM saw)
                protectInvocation = galileoService.invokeProtect(userInput, response, contextText, originalLlmResponse, "output", traceId, fullMessages);
            ...
            // Log the interaction to Galileo
            if (galileoService.isLoggingEnabled() && response != null) {
                ...
                Double protectProbability = (protectInvocation != null && protectInvocation.getResult() != null) 
                    ...
                    : null;
                // Extract tool span from Protect invocation
                Map<String, Object> protectSpan = (protectInvocation != null) ? protectInvocation.getToolSpan() : null;
                galileoService.logChatInteraction(userInput, response, provider, modelName, durationNs, traceId, retrieverSpan, originalForMetadata, protectProbability, null, protectSpan, fullMessages);
            }
            ...
            if (galileoService.isLoggingEnabled()) {
                galileoService.logChatInteraction(userInput, "Error: " + errorMessage, provider, modelName, durationNs, traceId, null, null, null, null, null, null);
            }
